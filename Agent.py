...
class Agent:
	...
	def take_action(self, environment):
		# use the epsilon-greedy strategy to choose
		# a random action
				...
				for j in range(LENGTH):
					if environment.is_board_empty(i, j):
						...
						if self.Values[state] > best_value:
							...
							best_state = state
							next_move = (i, j)
			#self.draw_values(environment, position_to_value)
	...
	def update(self, environment):
		...
		for previous in reversed(self.state_history):
			value = self.Values[previous] + self.alpha * (target - self.Values[previous])
